...
--buff:write(para,...) - zbuff写数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
...
--buff:read(length) - zbuff读数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
--int 读取buff中的字节数
--返回值：
--string 读取结果
...
--buff:clear(num) - zbuff清空数据（与当前指针位置无关；执行后指针位置不变）
--传入值：
...
--buff:seek(base,offset) - zbuff设置光标位置（可能与当前指针位置有关；执行后指针会被设置到指定位置）
--传入值：
--int 偏移长度
...
function buff:seek(base,offset) end

--buff:pack(format,val1, val2,...) - 将一系列数据按照格式字符转化，并写入（从当前指针位置开始；执行后指针会向后移动）
--传入值：
...
--buff:unpack(format) - 将一系列数据按照格式字符读取出来（从当前指针位置开始；执行后指针会向后移动）
--传入值：
...
--buff:readI8() - 读取一个指定类型的数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
...
--buff:readU32() - 读取一个指定类型的数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
--注释 读取类型可为：I8、U8、I16、U16、I32、U32、I64、U64、F32、F64
--返回值：
--number 读取的数据，如果越界则为nil
...
--buff:writeI8() - 写入一个指定类型的数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
--number 待写入的数据
...
--buff:wwriteU32() - 写入一个指定类型的数据（从当前指针位置开始；执行后指针会向后移动）
--传入值：
--number 待写入的数据
--注释 写入类型可为：I8、U8、I16、U16、I32、U32、I64、U64、F32、F64
--返回值：
...
--buff:toStr(offset,length) - 按起始位置和长度取出数据（与当前指针位置无关；执行后指针位置不变）
--传入值：
...
--buff:len() - 获取zbuff对象的长度（与当前指针位置无关；执行后指针位置不变）
--返回值：
--int zbuff对象的长度
--例子：
--len = buff:len()
...
--buff:setFrameBuffer(width,height,bit,color) - 设置buff对象的FrameBuffer属性（与当前指针位置无关；执行后指针位置不变）
--传入值：
--int FrameBuffer的宽度
--int FrameBuffer的高度
...
--buff:pixel(x,y,color) - 设置或获取FrameBuffer某个像素点的颜色（与当前指针位置无关；执行后指针位置不变）
--传入值：
...
--buff:drawLine(x1,y1,x2,y2,color) - 画一条线（与当前指针位置无关；执行后指针位置不变）
--传入值：
...
--buff:drawRect(x1,y1,x2,y2,color,fill) - 画一个矩形（与当前指针位置无关；执行后指针位置不变）
--传入值：
--int 起始坐标点与最左边的距离，范围是0~宽度-1
--int 起始坐标点与最上边的距离，范围是0~高度-1
--int 结束坐标点与最左边的距离，范围是0~宽度-1
...
--buff:drawCircle(x,y,r,color,fill) - 画一个圆形（与当前指针位置无关；执行后指针位置不变）
--传入值：
...
--buff:free() - 释放zbuff所申请内存 注意：gc时会自动释放zbuff以及zbuff所申请内存，所以通常无需调用此函数，调用前请确认您已清楚此函数用处！调用此函数并不会释放掉zbuff，仅会释放掉zbuff所申请的内存，zbuff需等gc时自动释放！！！
--例子：
--buff:free()
function buff:free() end

--buff:resize(n) - 调整zbuff实际分配空间的大小，类似于realloc的效果，new = realloc(old, n)，可以扩大或者缩小（如果缩小后len小于了used，那么used=新len）
--传入值：
--int 新空间大小
--例子：
--buff:resize(20)
...
--buff:copy(start, para,...) - zbuff动态写数据，类似于memcpy效果，当原有空间不足时动态扩大空间
--传入值：
...
--buff:used() - 获取zbuff里最后一个数据位置指针到首地址的偏移量，来表示zbuff内已有有效数据量大小，注意这个不同于分配的空间大小，由于seek()会改变最后一个数据位置指针，因此也会影响到used()返回值。
--返回值：
--int 有效数据量大小
--例子：
--buff:used()
...
--buff:del(offset,length) - 删除zbuff 0~used范围内的一段数据，注意只是改变了used的值，并不是真的在ram里去清除掉数据
--传入值：
...
--buff:query(offset,length,isbigend,issigned,isfloat) - 按起始位置和长度0~used范围内取出数据，如果是1,2,4,8字节，根据后续参数转换成浮点或者整形
--传入值：
--int 数据的起始位置（起始位置为0）
--int 数据的长度
...
--buff:set(start, num, len) - zbuff的类似于memset操作，类似于memset(&buff[start], num, len)，当然有ram越界保护，会对len有一定的限制
--传入值：
--int 可选，开始位置，默认为0,
--int 可选，默认为0。要设置为的值
...
--buff:isEqual(start, buff2, start2, len) - zbuff的类似于memcmp操作，类似于memcmp(&buff[start], &buff2[start2], len)
--传入值：
--int 可选，开始位置，默认为0,
--zbuff 比较的对象
