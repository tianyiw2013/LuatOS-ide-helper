...
--ioqueue.init(hwtimer_id,cmd_cnt,repeat_cnt) - 初始化一个io操作队列
--传入值：
...
--ioqueue.setdelay(hwtimer_id,time_us,time_tick,continue) - 对io操作队列增加延时命令
--传入值：
--int 硬件定时器id
--int 延时时间,0~65535us
...
--ioqueue.delay(hwtimer_id) - 对io操作队列增加一次重复延时，在前面必须有setdelay且是连续延时
--传入值：
--int 硬件定时器id
--return 无
--例子：
...
--ioqueue.setgpio(hwtimer_id,pin,is_input,pull_mode,init_level) - 对io操作队列增加设置gpio命令
--传入值：
--int 硬件定时器id
--int pin
--boolean 是否是输入
...
--ioqueue.input(hwtimer_id,pin) - 对io操作队列增加读取gpio命令
--传入值：
--int 硬件定时器id
--int pin
--return 无
...
--ioqueue.output(hwtimer_id,pin,level) - 对io操作队列增加输出GPIO命令
--传入值：
--int 硬件定时器id
--int pin
--int 输出电平
...
--ioqueue.set_cap(hwtimer_id,pin,pull_mode,irq_mode,max_tick) - 对io操作队列增加设置捕获某个IO命令
--传入值：
--int 硬件定时器id
--int pin
--int 上下拉模式,只能是0,gpio.PULLUP,gpio.PULLDOWN
...
--ioqueue.capture(hwtimer_id) - 对io操作队列增加捕获一次IO状态命令
--传入值：
--int 硬件定时器id
--return 无
--例子：
...
--ioqueue.capend(hwtimer_id,pin) - 对io操作队列增加结束捕获某个IO命令
--传入值：
--int 硬件定时器id
--int pin
--return 无
...
function ioqueue.capend(hwtimer_id,pin) end

--ioqueue.get(hwtimer_id, input_buff, capture_buff) - * 获取io操作队列中输入和捕获的数据
--传入值：
--int 硬件定时器id
...
--ioqueue.start(hwtimer_id) - 启动io操作队列
--传入值：
--int 硬件定时器id
--return 无
--例子：
...
--ioqueue.stop(hwtimer_id) - 停止io操作队列，可以通过start从头开始
--传入值：
--int 硬件定时器id
--返回值：
...
--ioqueue.release(hwtimer_id) - 释放io操作队列的资源，下次使用必须重新init
--传入值：
--int 硬件定时器id
--return 无
--例子：
...
--ioqueue.clear(hwtimer_id) - 清空io操作队列
--传入值：
--int 硬件定时器id
--return 无
--例子：
...
--ioqueue.done(hwtimer_id) - 检测io操作队列是否已经执行完成
--传入值：
--int 硬件定时器id
--返回值：
--boolean 队列是否执行完成，
...
--ioqueue.exti(pin,pull_mode,irq_mode,onoff) - 启动/停止一个带系统tick返回的外部中断
--传入值：
--int pin
--int 上下拉模式,只能是0,gpio.PULLUP,gpio.PULLDOWN
--int 中断模式,只能是gpio.BOTH,gpio.RISING,gpio.FALLING
