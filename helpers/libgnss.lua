...
--libgnss.getIntLocation(speed_type) - 获取位置信息
--传入值：
--int 速度单位,默认是m/h
--返回值：
...
--libgnss.getRmc(data_mode) - 获取原始RMC位置信息
--传入值：
...
--libgnss.getGsa(data_mode) - 获取原始GSA信息
--传入值：
--int 模式
--返回值：
--table 原始GSA数据
...
--libgnss.getVtg(data_mode) - 获取VTA速度信息
--传入值：
...
--libgnss.getGga(data_mode) - 获取GGA数据
--传入值：
...
--libgnss.getGll(data_mode) - 获取GLL数据
--传入值：
...
--libgnss.bind(id, next_id) - 绑定uart端口进行GNSS数据读取
--传入值：
--int uart端口号
...
--libgnss.rtcAuto(enable) - 定位成功后自动设置RTC
--传入值：
...
--libgnss.casic_aid(dt, loc) - 合成Air530Z所需要的辅助定位数据
--传入值：
--table 时间信息
--table 经纬度及海拔
--返回值：
