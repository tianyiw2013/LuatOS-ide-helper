...
--nimble.init(name) - 初始化BLE上下文,开始对外广播/扫描
--传入值：
...
--nimble.send_msg(conn, handle, data) - 发送信息
--传入值：
--int 连接id, 当前固定填1
--int 处理id, 当前固定填0
...
--nimble.setUUID(tp, addr) - 设置server/peripheral的UUID
--传入值：
...
--nimble.sendNotify(srv_uuid, chr_uuid, data) - 发送notify
--传入值：
...
--nimble.sendIndicate(srv_uuid, chr_uuid, data) - 发送indicate
--传入值：
--string 服务的UUID,预留,当前填nil就行
--string 特征的UUID,必须填写
--string 数据, 必填, 跟MTU大小相关, 一般不要超过256字节
...
--nimble.advParams(conn_mode, disc_mode, itvl_min, itvl_max, channel_map, filter_policy, high_duty_cycle) - 设置广播参数
--传入值：
...
--nimble.setChr(index, uuid, flags) - 设置chr的特征
--传入值：
--int chr的索引, 默认0-3
...
--nimble.config(id, value) - 设置chr的特征
--传入值：
--int 配置的id,请查阅常量表
--any 根据配置的不同, 有不同的可选值
--返回值：
...
--nimble.ibeacon(data, major, minor, measured_power) - 配置iBeacon的参数,仅iBeacon模式可用
--传入值：
--string 数据, 必须是16字节
...
--nimble.advData(data, flags) - 配置广播数据,仅iBeacon模式可用
--传入值：
...
--nimble.discChr(svr_uuid) - 扫描从机的指定服务的特征值
--传入值：
--string 指定服务的UUID值
--返回值：
--boolean 成功启动扫描与否
...
--nimble.listChr(svr_uuid) - 获取从机的指定服务的特征值列表
--传入值：
--string 指定服务的UUID值
--返回值：
...
--nimble.discDsc(svr_uuid, chr_uuid) - 扫描从机的指定服务的特征值的其他属性
--传入值：
--string 指定服务的UUID值
--string 特征值的UUID值
--返回值：
...
--nimble.writeChr(svr_uuid, chr_uuid, data) - 往指定的服务的指定特征值写入数据
--传入值：
--string 指定服务的UUID值
--string 指定特征值的UUID值
--string 待写入的数据
...
--nimble.readChr(svr_uuid, chr_uuid) - 从指定的服务的指定特征值读取数据(异步)
--传入值：
--string 指定服务的UUID值
--string 指定特征值的UUID值
--返回值：
...
--nimble.subChr(svr_uuid, chr_uuid) - 订阅指定的服务的指定特征值
--传入值：
--string 指定服务的UUID值
--string 指定特征值的UUID值
--返回值：
...
--nimble.unsubChr(svr_uuid, chr_uuid) - 取消订阅指定的服务的指定特征值
--传入值：
--string 指定服务的UUID值
--string 指定特征值的UUID值
--返回值：
