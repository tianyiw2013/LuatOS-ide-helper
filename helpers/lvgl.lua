...
--lvgl.draw_mask_radius_param_t() - 创建一个lv_draw_mask_radius_param_t
--返回值：
...
--lvgl.draw_mask_radius_param_t_free(radius) - 释放一个lv_draw_mask_radius_param_t
--例子：
--local lvgl.draw_mask_radius_param_t_free(radius)
function lvgl.draw_mask_radius_param_t_free(radius) end

--lvgl.draw_mask_line_param_t() - 创建一个lv_draw_mask_line_param_t
--返回值：
...
--lvgl.draw_mask_line_param_t_free(line) - 释放一个lv_draw_mask_line_param_t
--例子：
--local lvgl.draw_mask_line_param_t_free(line)
function lvgl.draw_mask_line_param_t_free(line) end

--lvgl.draw_mask_fade_param_t() - 创建一个lv_draw_mask_fade_param_t
--返回值：
...
--lvgl.font_load(path/spi_device,size,bpp,thickness,cache_size,sty_zh,sty_en) - 从文件系统加载字体
--传入值：
...
--lvgl.font_free(font) - 释放字体,慎用!!!仅通过font_load加载的字体允许卸载,通过font_get获取的字体不允许卸载
--传入值：
--string 字体路径
--返回值：
--userdata 字体指针
...
--lvgl.obj_set_event_cb(obj, func) - 设置组件的事件回调
--传入值：
--userdata lvgl组件指针
...
--lvgl.obj_set_signal_cb(obj, func) - 设置组件的信号回调
--传入值：
--userdata lvgl组件指针
...
--lvgl.anim_set_exec_cb(anim, func) - 设置动画回调
--传入值：
--userdata 动画指针
--userdata lvgl组件指针
...
--lvgl.anim_set_ready_cb(anim, func) - 设置动画回调
--传入值：
--userdata 动画指针
--userdata lvgl组件指针
...
--lvgl.anim_path_set_cb(path, func) - 设置动画回调
--传入值：
--userdata 动画指针
--userdata lvgl组件指针
...
--lvgl.event_send(obj, ent) - 发送事件给组件
--传入值：
--userdata 组件指针
...
--lvgl.scr_load_anim(scr) - 载入指定的screen并使用指定的转场动画
--传入值：
--userdata screen指针
--例子：
--    local scr = lvgl.obj_create(nil, nil)
...
--lvgl.sleep(enable) - LVGL休眠控制，暂停/恢复刷新定时器，目前只有105和EC618可以用
--传入值：
...
--lvgl.init(w, h, lcd, buff_size, buff_mode) - 初始化LVGL
--传入值：
--int 屏幕宽,可选,默认从lcd取
--int 屏幕高,可选,默认从lcd取
...
--lvgl.anim_free(anim) - 释放一个anim
--例子：
--local lvgl.anim_free(anim)
function lvgl.anim_free(anim) end

--lvgl.anim_path_t() - 创建一个lv_anim_path_t
--返回值：
...
--lvgl.anim_path_t_free(anim_path_t) - 释放一个lv_anim_path_t
--例子：
--local lvgl.anim_path_t_free(anim_path_t)
function lvgl.anim_path_t_free(anim_path_t) end

--lvgl.anim_set_path_str(anim, tp) - 设置动画路径方式
--传入值：
--userdata 动画指针
...
--lvgl.qrcode_create(parent, size, dark_color, light_color) - 创建qrcode组件
--传入值：
--userdata 父组件
--int 长度,因为qrcode是正方形
...
--lvgl.qrcode_update(qrcode, cnt) - 设置qrcode组件的二维码内容,配合qrcode_create使用
--传入值：
...
--lvgl.qrcode_delete(qrcode) - 删除qrcode组件
--传入值：
--userdata qrcode组件,由qrcode_create创建
--返回值：
--nil 无返回值
...
--lvgl.style_create() - 创建一个style并初始化
--返回值：
--userdata style指针
--例子：
...
--lvgl.style_list_create() - 创建一个style_list
--返回值：
--userdata style指针
--例子：
...
--lvgl.style_list_t() - 创建一个style_list
--返回值：
--userdata style指针
--例子：
...
--lvgl.style_delete(style) - 删除style,慎用,通常不会执行删除操作
--传入值：
--userdata style指针
--例子：
--local style = lvgl.style_create()
...
--lvgl.style_list_delete(style) - 删除style_list,慎用,通常不会执行删除操作
--传入值：
--userdata style指针
--例子：
--local style_list = lvgl.style_list_create()
...
--lvgl.demo_benchmark() - lvgl benchmark demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.demo_keypad_encoder() - lvgl keypad_encoder demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.demo_music() - lvgl music demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.demo_printer() - lvgl printer demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.demo_stress() - lvgl stress demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.demo_widgets() - lvgl widgets demo
--返回值：
--nil 无返回值
--例子：
--lvgl.init()
...
--lvgl.indev_drv_register(tp, dtp) - 注册输入设备驱动
--传入值：
...
--lvgl.indev_point_emulator_update(x, y, state) - 更新模拟输入设备的坐标数据
--传入值：
...
--lvgl.indev_kb_update(key) - 更新键盘输入设备的按键值
--传入值：
--int 按键值，默认为0，按键抬起
--返回值：
--nil 无返回值
...
--lvgl.gif_create(parent, path) - 创建gif组件
--传入值：
...
--lvgl.gif_restart(gif) - 重新播放gif组件
--传入值：
