...
--socket.sntp(sntp_server) - sntp时间同步
--传入值：
...
--    socket.sntp()
--end)
function socket.sntp(sntp_server) end

--socket.ntptm() - 网络对时后的时间戳(ms级别)
--返回值：
--table 包含时间信息的数据
--例子：
---- 本API于 2023.11.15 新增
...
--socket.sntp_port(port) - 设置SNTP服务器的端口号
--传入值：
--int port 端口号, 默认123
--返回值：
--int 返回当前的端口号
...
--socket.create(adapter, cb) - 在某个适配的网卡上申请一个socket_ctrl
--传入值：
--int 适配器序号， 只能是socket.ETH0（外置以太网），socket.LWIP_ETH（内置以太网），socket.LWIP_STA（内置WIFI的STA），socket.LWIP_AP（内置WIFI的AP），socket.LWIP_GP（内置蜂窝网络的GPRS），socket.USB（外置USB网卡），如果不填，优先选择soc平台自带能上外网的适配器，若仍然没有，选择最后一个注册的适配器
...
--socket.debug(ctrl, onoff) - 配置是否打开debug信息
--传入值：
...
--socket.connect(ctrl, ip, remote_port, need_ipv6_dns) - 作为客户端连接服务器
--传入值：
--user_data socket.create得到的ctrl
...
--socket.tx(ctrl, data, ip, port, flag) - 发送数据给对端，UDP单次发送不要超过1460字节，否则很容易失败
--传入值：
--user_data socket.create得到的ctrl
...
--socket.rx(ctrl, buff, flag, limit) - 接收对端发出的数据，注意数据已经缓存在底层，使用本函数只是提取出来，UDP模式下一次只会取出一个数据包
--传入值：
--user_data socket.create得到的ctrl
...
--socket.read(netc, len) - 读取数据(非zbuff版本)
--传入值：
...
--socket.wait(ctrl) - 等待新的socket消息，在连接成功和发送数据成功后，使用一次将network状态转换到接收新数据
--传入值：
--user_data socket.create得到的ctrl
--返回值：
--boolean true没有异常发生，false失败了，如果false则不需要看下一个返回值了，如果false，后续要close
...
--socket.accept(ctrl) - 作为服务端接收到一个新的客户端，注意，如果是类似W5500的硬件协议栈不支持1对多，则不需要第二个参数
--传入值：
...
--socket.state(ctrl) - 获取socket当前状态
--传入值：
--user_data socket.create得到的ctrl
--返回值：
...
--socket.release(ctrl) - 主动释放掉network_ctrl
--传入值：
--user_data socket.create得到的ctrl
--例子：
---- 释放后就不能再使用了
...
--socket.setDNS(adapter_index, dns_index, ip) - 设置DNS服务器
--传入值：
...
--socket.sslLog(log_level) - 设置SSL的log登记
--传入值：
--int mbedtls log等级
--返回值：
--nil 无返回值
...
--socket.adapter(index) - 查看网卡适配器的联网状态
--传入值：
