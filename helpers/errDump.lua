...
--errDump.dump(zbuff, type, isDelete) - 手动读取异常日志，主要用于用户将日志发送给自己的服务器而不是IOT平台，如果在errDump.config配置了周期上传，则不能使用本函数
--传入值：
...
--errDump.record(string) - 写入用户的异常日志，注意最大只有4KB，超过部分新的覆盖旧的，开启自动上传后会上传到合宙IOT平台
--传入值：
--string 日志
--返回值：
--nil 无返回值
...
--errDump.config(enable, period, user_flag, custom_id, host, port) - 配置关键日志上传IOT平台，这里的日志包括引起luavm异常退出的日志和用户通过record写入的日志，类似于air的errDump
--传入值：
...
function errDump.config(enable,period,user_flag,custom_id,host,port) end