...
--spi.setup(id, cs, CPHA, CPOL, dataw, bandrate, bitdict, ms, mode) - 设置并启用SPI
--传入值：
--int SPI号,例如0
...
--spi.createSoft(cs, mosi, miso, clk, CPHA, CPOL, dataw, bitdict, ms, mode) - 设置并启用软件SPI
--传入值：
...
--spi.transfer(id, send_data, send_len, recv_len) - 传输SPI数据
--传入值：
...
--spi.recv(id, size, buff) - 接收指定长度的SPI数据
--传入值：
--int SPI号,例如0
--int 数据长度
...
--spi.deviceSetup(id, cs, CPHA, CPOL, dataw, bandrate, bitdict, ms, mode) - 设置并启用SPI(对象方式)
--传入值：
--int SPI号,例如0
--int CS 片选脚,在w600不可用请填nil
--int CPHA 默认0,可选0/1
...
--spi.xfer(id, txbuff, rxbuff, rx_len, transfer_done_topic) - 非阻塞方式硬件SPI传输SPI数据，目的为了提高核心利用率。API直接返回是否启动传输，传输完成后通过topic回调，本API适合硬件SPI传输大量数据传输，外设功能（LCD SPI，W5500 SPI之类的）占据的SPI和软件SPI不能用，少量数据传输建议使用传统阻塞型API
--传入值：
