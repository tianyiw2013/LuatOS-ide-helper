...
--sensor.ds18b20(pin, check_crc) - 获取DS18B20的温度数据
--传入值：
--int gpio端口号
...
--sensor.w1_reset(pin) - 单总线协议,复位设备
--传入值：
--int gpio端口号
--返回值：
--nil 无返回
...
--sensor.w1_connect(pin) - 单总线协议,连接设备
--传入值：
--int gpio端口号
--返回值：
...
--sensor.w1_write(pin, data1,data2) - 单总线协议,往总线写入数据
--传入值：
--int gpio端口号
--int 第一个数据
--int 第二个数据, 可以写N个数据
...
--sensor.w1_read(pin, len) - 单总线协议,从总线读取数据
--传入值：
--int gpio端口号
--int 读取的长度
--返回值：
...
--sensor.hx711(pin_date,pin_clk) - 获取Hx711的压力传感数据
--传入值：
--int 数据的gpio端口号
--int 时钟的gpio端口号
--返回值：
...
--sensor.cs1237(pin_date,pin_clk) - 获取cs1237传感数据
--传入值：
--int 数据的gpio端口号
--int 时钟的gpio端口号
--返回值：
...
--sensor.ws2812b(pin,data,T0H,T0L,T1H,T1L) - 设置ws2812b输出(gpio驱动方式)
--传入值：
--int ws2812b的gpio端口号
...
--sensor.ws2812b_pwm(pin,data) - 设置ws2812b输出(pwm驱动方式,需要pwm能输出800k频率，否则无法使用此方法)
--传入值：
--int pwm端口号
--string/zbuff 待发送的数据（如果为zbuff数据，则会无视指针位置始终从0偏移开始）
--例子：
...
--sensor.ws2812b_spi(pin,data) - 设置ws2812b输出(spi驱动方式,需要spi能输出5M频率，否则无法使用此方法)
--传入值：
--int spi端口号
--string/zbuff 待发送的数据（如果为zbuff数据，则会无视指针位置始终从0偏移开始）
--例子：
...
--sensor.dht1x(pin) - 获取DHT11/DHT12的温湿度数据
--传入值：
--int gpio端口号
--boolean 是否校验crc值,默认为true. 不校验crc值能提高读取成功的概率,但可能会读取到错误的值
--返回值：
...
--sensor.sc12a(sda,scl) - 获取sc12a被触摸的通道数据
--传入值：
--int 数据的gpio端口号
--int 时钟的gpio端口号
--返回值：
...
--sensor.yhm27xxx(pin, chip_id, reg, data) - 单总线命令读写YHM27XX
--传入值：
--int gpio端口号
--int 芯片ID
--int 寄存器地址
