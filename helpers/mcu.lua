...
--mcu.unique_id() - 获取设备唯一id. 注意,可能包含不可见字符,如需查看建议toHex()后打印
--返回值：
...
--mcu.ticks() - 获取启动后的tick数,本身是无符号值,范围0~0xffffffff,lua是有符号计算,计算时超过0x7fffffff会变负数
--返回值：
--int 当前tick值
--例子：
...
--mcu.reg32(address, value, mask) - 读写mcu的32bit寄存器或者ram,谨慎使用写功能,请熟悉mcu的寄存器使用方法后再使用
--传入值：
--int 寄存器或者ram地址
...
--mcu.x32(value) - 转换10进制数为16进制字符串输出
--传入值：
--int 需要转换的值
--返回值：
--string 16进制字符串
...
--mcu.tick64() - 获取启动后的高精度tick，如果支持bit64库，可以直接输出转换好的bit64结构
--传入值：
...
--mcu.dtick64(tick1, tick2, check_value) - 计算2个64bit tick的差值
--传入值：
--string 64bit的string
--string 64bit的string
...
--mcu.setXTAL(source_main, source_32k, delay) - 选择时钟源,当前仅air105支持
--传入值：
...
--mcu.hardfault(mode) - mcu死机时处理模式，目前只有EC618平台适用
--传入值：
...
--mcu.iomux(type, channel, value) - 在外设打开前，将外设IO复用到非默认配置上，目前只支持Air780E的部分外设复用到其他配置，这是一个临时接口，如果后续有更合适的api，本接口将不再更新
--传入值：
...
--mcu.altfun(type, sn, pad_index, alt_fun, is_input) - IO外设功能复用选择，注意普通MCU是以GPIO号为序号，但是专用SOC，比如CAT1的，是以PAD号为序号。本函数不是所有平台适用
--传入值：
...
--mcu.XTALRefOutput(source_main, source_32k) - 晶振参考时钟输出
--传入值：
