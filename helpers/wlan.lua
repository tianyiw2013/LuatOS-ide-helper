...
--wlan.ready() - 作为STATION时,是否已经连接上AP,且获取IP成功
--返回值：
--bool 已经连接成功返回true,否则返回false
function wlan.ready() end

--wlan.connect(ssid, password, auto_reconnect, bssid) - 作为STATION时,连接到指定AP
--传入值：
--string AP的ssid
...
--wlan.getIP() - 获取ip,仅STATION或APSTA模式下有意义
--返回值：
--string ip地址,当前仅返回ipv4地址,例如 "192.168.1.25"
function wlan.getIP() end

--wlan.createAP(ssid, passwd, gateway, netmask, channel, opts) - 启动AP
--传入值：
--string AP的SSID,必填
--string AP的密码,可选
...
--wlan.getInfo() - 获取信息,如AP的bssid,信号强度, STA联网后可获取
--返回值：
--table 详情,键值对形式
--例子：
--
...
--wlan.hostname(new_name) - 读取或设置Hostname
--传入值：
...
function wlan.hostname(new_name) end

--wlan.staIp(dhcp_enable, ip, netmask, gateway) - 设置Station模式下的IP获取模式
--传入值：
