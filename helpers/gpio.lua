...
--gpio.setup(pin, mode, pull, irq, alt) - 设置管脚功能
--传入值：
...
function gpio.setup(pin,mode,pull,irq,alt) return function() end end

--gpio.caplevel(pin, level,func) - 捕获管脚电平持续时长，单位us
--传入值：
--int pin GPIO编号,必须是数值
...
--gpio.close(pin) - 关闭管脚功能(高阻输入态),关掉中断
--传入值：
--int pin GPIO编号,必须是数值
--返回值：
--nil 无返回值,总是执行成功
...
--gpio.setDefaultPull(val) - 设置GPIO脚的默认上拉/下拉设置, 默认是平台自定义(一般为开漏).
--传入值：
...
--gpio.toggle(pin) - 变换GPIO脚输出电平,仅输出模式可用
--传入值：
--int 管脚的GPIO号
--返回值：
--nil 无返回值
...
--gpio.pulse(pin,level,len,delay) - 在同一个GPIO输出一组脉冲, 注意, len的单位是bit, 高位在前.
--传入值：
--int gpio号
--int/string 数值或者字符串.
...
--gpio.debounce(pin, ms, mode) - 防抖设置, 根据硬件ticks进行防抖
--传入值：
...
--gpio.count(pin) - 获取gpio中断数量，并清空累计值，类似air724的脉冲计数
--传入值：
--int gpio号, 0~127, 与硬件相关
--返回值：
