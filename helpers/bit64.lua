...
--bit64.to32(data64bit) - 64bit数据转成32bit输出
--传入值：
--string 9字节数据
--返回值：
...
--bit64.to64(data32bit) - 32bit数据转成64bit数据
--传入值：
--int/number 32bit数据
--返回值：
--string 9字节数据
...
--bit64.show(a,type,flag) - 64bit数据格式化打印成字符串，用于显示值
--传入值：
--string 需要打印的64bit数据
...
--bit64.plus(a,b,flag1,flag2) - 64bit数据加,a+b,a和b中有一个为浮点，则按照浮点运算
--传入值：
--string a
--string/int/number b
...
--bit64.minus(a,b,flag1,flag2) - 64bit数据减,a-b,a和b中有一个为浮点，则按照浮点运算
--传入值：
--string a
--string/int/number b
--boolean 整形运算时是否按照无符号方式，true是，false不是，默认false，浮点运算忽略
...
--bit64.multi(a,b,flag1,flag2) - 64bit数据乘,a*b,a和b中有一个为浮点，则按照浮点运算
--传入值：
--string a
--string/int/number b
--boolean 整形运算时是否按照无符号方式，true是，false不是，默认false，浮点运算忽略
...
--bit64.pide(a,b,flag1,flag2) - 64bit数据除,a/b,a和b中有一个为浮点，则按照浮点运算
--传入值：
--string a
--string/int/number b
--boolean 整形运算时是否按照无符号方式，true是，false不是，默认false，浮点运算忽略
...
--bit64.shift(a,b,flag) - 64bit数据位移 a>>b 或者 a<<b
--传入值：
--string a
--int b
...
--bit64.strtoll(data, base) - 将字符串转为LongLong数据
--传入值：
--string 待转换的数据,必须存在
