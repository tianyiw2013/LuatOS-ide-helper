...
--- 作为服务端开始监听
---@param ctrl user_data socket.create得到的ctrl
---@param multi_client number 等待accept的客户端数量, 默认0不支持, 取值范围0~8
---@return boolean #1 true没有异常发生，false失败了，如果false则不需要看下一个返回值了，如果false，后续要close
---@return boolean #2 true已经connect，false没有connect，之后需要接收socket.ON_LINE消息
--- ```lua
...
--- ```
function socket.listen(ctrl, multi_client) end

--- 作为服务端接收到一个新的客户端
---@param ctrl user_data socket.create得到的ctrl，这里是服务器端
---@param args string or function or nil string为消息通知的taskName，function则为回调函数，和socket.create参数一致
---@return boolean #1 true没有异常发生，false失败了，如果false则不需要看下一个返回值了，如果false，后续要close
---@return user_data #2 or nil 如果支持1对多，则会返回新的ctrl，自动create，如果不支持则返回nil
---@return string #3 客户端ip, 2025.11.10新增
---@return number #4 客户端port, 2025.11.10新增
--- ```lua
...
--- ```
function socket.accept(ctrl, args) end
