---@meta

--- LuatOS IDE Helper for module: rtmp
--- ```yaml
--- Summary: RTMP 直播推流
--- URL: https://gitee.com/openLuat/LuatOS/tree/master/luat/../components/rtmp/binding/luat_lib_rtmp.c
--- Demo: 
--- Video: 
--- Tag: LUAT_USE_RTMP
--- ```
--- ```lua
--- RTMP推流示例
--- local rtmp = rtmp.create("rtmp://example.com:1935/live/stream")
--- rtmp:setCallback(function(state, ...)
---     if state == rtmp.STATE_CONNECTED then
---         print("已连接到推流服务器")
---     elseif state == rtmp.STATE_PUBLISHING then
---         print("已开始推流")
---     elseif state == rtmp.STATE_ERROR then
---         print("出错:", ...)
---     end
--- end)
--- rtmp:connect()
--- 开始处理
--- rtmp:start()
--- 30秒后停止
--- sys.wait(30000)
--- rtmp:stop()
--- 断开连接
--- rtmp:disconnect()
--- rtmp:destroy()
--- ```
rtmp = {}

--- 创建RTMP推流上下文
---@param url string url RTMP服务器地址, 格式: rtmp://host:port/app/stream
---@return userdata #1 RTMP上下文对象
--- ```lua
--- local rtmp = rtmp.create("rtmp://example.com:1935/live/stream")
--- ```
function rtmp.create(url) end

--- 设置RTMP状态回调函数
---@param func function func 回调函数, 参数为 (state, ...)
---@return nil #1 无返回值
--- ```lua
--- rtmp:setCallback(function(state, ...)
---     if state == rtmp.STATE_IDLE then
---         print("空闲状态")
---     elseif state == rtmp.STATE_CONNECTING then
---         print("正在连接")
---     elseif state == rtmp.STATE_HANDSHAKING then
---         print("握手中")
---     elseif state == rtmp.STATE_CONNECTED then
---         print("已连接")
---     elseif state == rtmp.STATE_PUBLISHING then
---         print("推流中")
---     elseif state == rtmp.STATE_DISCONNECTING then
---         print("正在断开")
---     elseif state == rtmp.STATE_ERROR then
---         print("错误:", ...)
---     end
--- end)
--- ```
function rtmp:setCallback(func) end

--- 连接到RTMP服务器
---@return boolean #1 成功返回true, 失败返回false
--- ```lua
--- local ok = rtmp:connect()
--- if ok then
---     print("连接请求已发送")
--- else
---     print("连接失败")
--- end
--- ```
function rtmp:connect() end

--- 断开RTMP连接
---@return boolean #1 成功返回true, 失败返回false
--- ```lua
--- rtmp:disconnect()
--- ```
function rtmp:disconnect() end

--- 处理RTMP事件
---@return nil #1 无返回值
--- ```lua
--- rtmp:start()
--- ```
function rtmp:start() end

--- 获取RTMP连接状态
---@return number #1 当前状态值
--- ```lua
--- local state = rtmp:getState()
--- if state == rtmp.STATE_CONNECTED then
---     print("已连接")
--- elseif state == rtmp.STATE_PUBLISHING then
---     print("正在推流")
--- end
--- ```
function rtmp:getState() end

--- 获取RTMP统计信息
---@return table #1 统计信息表
--- ```lua
--- local stats = rtmp:getStats()
--- print("已发送字节数:", stats.bytes_sent)
--- print("已发送视频帧数:", stats.video_frames_sent)
--- print("已发送音频帧数:", stats.audio_frames_sent)
--- ```
function rtmp:getStats() end

--- 销毁RTMP上下文，释放所有资源
---@return nil #1 无返回值
--- ```lua
--- rtmp:destroy()
--- ```
function rtmp:destroy() end
